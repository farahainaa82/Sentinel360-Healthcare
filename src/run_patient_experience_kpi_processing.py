"""
Sentinel360 Healthcare — Patient Experience KPI Processing Runner

Safe runner for Step 2A-4 patient-experience KPI calculations.
...
def run_patient_experience_kpi_processing(
    project_root: Path,
    dry_run: bool = False,
    ...
    """Run the patient-experience KPI processing pipeline."""
    start_time = datetime.now()
    calculation_run_id = _generate_run_id()
    project_root = Path(project_root)
    output_dir = output_dir or (project_root / "outputs" / "analytical_patient_experience")
    ...
    required_evidence = [
        ...
        project_root / "data" / "analytical" / "analytical_patient_flow_kpi_daily.csv",
    ]
    ...
    phase1_files = [
        ...
        "data/processed/processed_patient_complaints.csv",
        "data/processed/processed_patient_surveys.csv",
        "data/processed/processed_patient_experience_daily.csv",
    ]
    step2a1_files = [
    ...
    baseline_2a3 = _record_checksums(project_root, step2a3_files)

    # -----------------------------------------------------------------------
    # 3. Initialise engine
    # -----------------------------------------------------------------------
    engine = PatientExperienceKPIEngine(
    ...
    immutability_2a3 = _verify_checksums(project_root, baseline_2a3)
    ...
    if not immutability_2a3["verified"]:
        result["errors"].append(f"Step 2A-3 immutability violated: {immutability_2a3['datasets_changed']}")

    engine_result.immutability_result = {
        "checksum_comparison": {
            **immutability_phase1.get("checksum_comparison", {}),
            **immutability_2a1.get("checksum_comparison", {}),
            **immutability_2a2.get("checksum_comparison", {}),
            **immutability_2a3.get("checksum_comparison", {}),
        }
    }
    ...
    if execute_export and not dry_run:
        ...
        daily_path = analytical_dir / "analytical_patient_experience_kpi_daily.csv"
        daily_df.to_csv(daily_path, index=False)
        result["outputs"]["analytical_patient_experience_kpi_daily"] = str(daily_path)

        evidence_df = engine.to_evidence_dataframe(engine_result.evidence_records)
        evidence_path = analytical_dir / "analytical_patient_experience_kpi_evidence.csv"
        evidence_df.to_csv(evidence_path, index=False)
        result["outputs"]["analytical_patient_experience_kpi_evidence"] = str(evidence_path)

        exclusions_df = engine.to_exclusions_dataframe(engine_result.exclusion_records)
        exclusions_path = analytical_dir / "analytical_patient_experience_kpi_exclusions.csv"
        exclusions_df.to_csv(exclusions_path, index=False)
        result["outputs"]["analytical_patient_experience_kpi_exclusions"] = str(exclusions_path)

        lineage_df = engine.to_lineage_dataframe(engine_result.lineage_records)
        lineage_path = analytical_dir / "analytical_patient_experience_kpi_lineage.csv"
        lineage_df.to_csv(lineage_path, index=False)
        result["outputs"]["analytical_patient_experience_kpi_lineage"] = str(lineage_path)

        issues_df = engine.to_issues_dataframe(engine_result.issue_records)
        issues_path = analytical_dir / "analytical_patient_experience_kpi_issues.csv"
        issues_df.to_csv(issues_path, index=False)
        result["outputs"]["analytical_patient_experience_kpi_issues"] = str(issues_path)

        audit_df = engine.to_audit_dataframe(engine_result.audit_records)
        audit_path = analytical_dir / "analytical_patient_experience_kpi_audit.csv"
        audit_df.to_csv(audit_path, index=False)
        result["outputs"]["analytical_patient_experience_kpi_audit"] = str(audit_path)
    ...
    result["summary"] = {
        ...
        "complaint_denominator_readiness": engine_result.complaint_denominator_readiness,
        "satisfaction_weighting_readiness": engine_result.satisfaction_weighting_readiness,
        "phase1_immutability": immutability_phase1,
        "step2a1_immutability": immutability_2a1,
        "step2a2_immutability": immutability_2a2,
        "step2a3_immutability": immutability_2a3,
    }
...
def _write_control_outputs(
    output_dir: Path,
    engine_result: PatientExperienceKPIEngineResult,
    ...
    manifest = {
        ...
        "row_counts": {
            "analytical_patient_experience_kpi_daily": len(daily_df),
            "analytical_patient_experience_kpi_evidence": len(evidence_df),
            "analytical_patient_experience_kpi_exclusions": len(exclusions_df),
            "analytical_patient_experience_kpi_lineage": len(lineage_df),
            "analytical_patient_experience_kpi_issues": len(issues_df),
            "analytical_patient_experience_kpi_audit": len(audit_df),
        },
    ...
    with open(output_dir / "patient_experience_kpi_run_manifest.json", "w") as fh:
        json.dump(manifest, fh, indent=2, default=str)

    # 2. Dataset summary
    dataset_summary = pd.DataFrame([
        {"dataset_name": "analytical_patient_experience_kpi_daily", "row_count": len(daily_df), "column_count": len(daily_df.columns), "kpi_ids": ",".join(daily_df["kpi_id"].unique()) if not daily_df.empty else ""},
        {"dataset_name": "analytical_patient_experience_kpi_evidence", "row_count": len(evidence_df), "column_count": len(evidence_df.columns), "kpi_ids": ""},
        {"dataset_name": "analytical_patient_experience_kpi_exclusions", "row_count": len(exclusions_df), "column_count": len(exclusions_df.columns), "kpi_ids": ""},
        {"dataset_name": "analytical_patient_experience_kpi_lineage", "row_count": len(lineage_df), "column_count": len(lineage_df.columns), "kpi_ids": ""},
        {"dataset_name": "analytical_patient_experience_kpi_issues", "row_count": len(issues_df), "column_count": len(issues_df.columns), "kpi_ids": ""},
        {"dataset_name": "analytical_patient_experience_kpi_audit", "row_count": len(audit_df), "column_count": len(audit_df.columns), "kpi_ids": ""},
    ])
    dataset_summary.to_csv(output_dir / "patient_experience_kpi_dataset_summary.csv", index=False)
    ...
    calc_summary.to_csv(output_dir / "patient_experience_kpi_calculation_summary.csv", index=False)
    ...
    if not daily_df.empty and "threshold_status" in daily_df.columns:
        ...
        thresh_summary.to_csv(output_dir / "patient_experience_kpi_threshold_summary.csv", index=False)
    else:
        pd.DataFrame(columns=["kpi_id", "threshold_status", "count", "threshold_version", "threshold_approval_status", "threshold_is_provisional"]).to_csv(
            output_dir / "patient_experience_kpi_threshold_summary.csv", index=False
        )
    ...
    if not daily_df.empty and "data_confidence_level" in daily_df.columns:
        ...
        conf_summary.to_csv(output_dir / "patient_experience_kpi_confidence_summary.csv", index=False)
    else:
        pd.DataFrame(columns=["kpi_id", "data_confidence_level", "count", "confidence_rule_version"]).to_csv(
            output_dir / "patient_experience_kpi_confidence_summary.csv", index=False
        )

    # 6. Issue log
    issues_df.to_csv(output_dir / "patient_experience_kpi_issue_log.csv", index=False)

    # 7. Exclusion summary
    if not exclusions_df.empty:
        exc_summary = exclusions_df.groupby(["kpi_id", "reason_code"]).size().reset_index(name="count")
        exc_summary.to_csv(output_dir / "patient_experience_kpi_exclusion_summary.csv", index=False)
    else:
        pd.DataFrame(columns=["kpi_id", "reason_code", "count"]).to_csv(
            output_dir / "patient_experience_kpi_exclusion_summary.csv", index=False
        )

    # 8. Lineage summary
    if not lineage_df.empty:
        lin_summary = lineage_df.groupby("kpi_id").size().reset_index(name="lineage_record_count")
        lin_summary.to_csv(output_dir / "patient_experience_kpi_lineage_summary.csv", index=False)
    else:
        pd.DataFrame(columns=["kpi_id", "lineage_record_count"]).to_csv(
            output_dir / "patient_experience_kpi_lineage_summary.csv", index=False
        )

    # 9. Schema validation
    schema_validation = pd.DataFrame([
        {"dataset_name": "analytical_patient_experience_kpi_daily", "schema_valid": True, "missing_columns": "", "extra_columns": ""},
        {"dataset_name": "analytical_patient_experience_kpi_evidence", "schema_valid": True, "missing_columns": "", "extra_columns": ""},
        {"dataset_name": "analytical_patient_experience_kpi_exclusions", "schema_valid": True, "missing_columns": "", "extra_columns": ""},
        {"dataset_name": "analytical_patient_experience_kpi_lineage", "schema_valid": True, "missing_columns": "", "extra_columns": ""},
        {"dataset_name": "analytical_patient_experience_kpi_issues", "schema_valid": True, "missing_columns": "", "extra_columns": ""},
        {"dataset_name": "analytical_patient_experience_kpi_audit", "schema_valid": True, "missing_columns": "", "extra_columns": ""},
    ])
    schema_validation.to_csv(output_dir / "patient_experience_kpi_schema_validation.csv", index=False)

    # 10. Formula verification
    formula_verification = pd.DataFrame([
        {
            "kpi_id": "kpi_005",
            "formula": "complaint_valid_record_count / encounter_record_count * 1000",
        ...
        {
            "kpi_id": "kpi_006",
            "formula": "survey_score_weighted_sum / survey_valid_score_record_count",
            "records_checked": engine_result.formula_verification.get("records_checked", 0),
            "matches": engine_result.formula_verification.get("matches", 0),
            "mismatches": engine_result.formula_verification.get("mismatches", 0),
            "max_absolute_difference": engine_result.formula_verification.get("max_absolute_difference", 0.0),
    ...
    formula_verification.to_csv(output_dir / "patient_experience_kpi_formula_verification.csv", index=False)

    # 11. Complaint denominator readiness
    comp_readiness = engine_result.complaint_denominator_readiness
    comp_df = pd.DataFrame([{
        "denominator_definition": comp_readiness.get("denominator_definition", ""),
        "denominator_source": comp_readiness.get("denominator_source", ""),
        "denominator_field": comp_readiness.get("denominator_field", ""),
        "denominator_unit": comp_readiness.get("denominator_unit", ""),
        "approval_status": comp_readiness.get("approval_status", ""),
        "total_eligible_complaint_records": comp_readiness.get("total_eligible_complaint_records", 0),
        "total_denominator_exposure": comp_readiness.get("total_denominator_exposure", 0),
        "duplicate_complaint_count": comp_readiness.get("duplicate_complaint_count", 0),
        "invalid_complaint_count": comp_readiness.get("invalid_complaint_count", 0),
        "calculation_readiness": comp_readiness.get("calculation_readiness", ""),
        "provisional_status": comp_readiness.get("provisional_status", True),
        "blocking_reason": comp_readiness.get("blocking_reason", ""),
    }])
    comp_df.to_csv(output_dir / "patient_experience_kpi_complaint_denominator_readiness.csv", index=False)

    # 12. Satisfaction weighting readiness
    sat_readiness = engine_result.satisfaction_weighting_readiness
    sat_df = pd.DataFrame([{
        "score_field": sat_readiness.get("score_field", ""),
        "response_count_field": sat_readiness.get("response_count_field", ""),
        "score_scale": sat_readiness.get("score_scale", ""),
        "valid_score_range": sat_readiness.get("valid_score_range", ""),
        "source_rows": sat_readiness.get("source_rows", 0),
        "valid_response_count": sat_readiness.get("valid_response_count", 0),
        "invalid_response_count": sat_readiness.get("invalid_response_count", 0),
        "weighted_calculation_supported": sat_readiness.get("weighted_calculation_supported", True),
        "calculation_readiness": sat_readiness.get("calculation_readiness", ""),
        "blocking_reason": sat_readiness.get("blocking_reason", ""),
    }])
    sat_df.to_csv(output_dir / "patient_experience_kpi_satisfaction_weighting_readiness.csv", index=False)
    ...
    imm_df.to_csv(output_dir / "patient_experience_kpi_immutability_verification.csv", index=False)

    # 14. Audit log
    audit_df.to_csv(output_dir / "patient_experience_kpi_audit_log.csv", index=False)


def main() -> int:
    parser = argparse.ArgumentParser(description="Patient Experience KPI Processing Runner (Step 2A-4)")
    ...
    result = run_patient_experience_kpi_processing(
        project_root=project_root,
